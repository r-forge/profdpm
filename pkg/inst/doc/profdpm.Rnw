...
The remainder of this article proceeds as follows: the relevant theory of product partition models is discussed in Section~\ref{PPMs}, the underlying \proglang{C} methods are detailed in Section~\ref{Code}, discussion and examples are presented in Sections~\ref{Examples:profLinear} and \ref{Examples:profBinary}. Finally, Section~\ref{Extensions} discusses some future directions for the \pkg{profdpm} package.\\
...
The following example simulates a dataset consisting of $99$ longitudinal measures on $33$ units of observation, or subjects. Each subject is measured at three times, drawn uniformly and independently from the unit interval. Each of the three measures per subject are drawn independently from the normal distribution with one of three linear mean functions of time, and with unit variance. The linear mean functions each have zero intercept and vary by slope. The longitudinal structure imposes a grouping among measurements on a single subject. Observations grouped in this way should always cluster together. Grouping structure is specified using the \code{group} parameter; a factor that behaves similarly to the \code{groups} parameter of \pkg{lattice} graphics functions. For the PPM of conjugate binary models, the grouping structure is imposed by the model formula. More specifically, grouped observations correspond to rows of the model matrix, resulting from a call to \code{model.matrix} on the formula passed to \code{profBinary}. Hence, the \code{profBinary} function does not have a \code{group} parameter in its prototype. 
...
<<label=figSim1, include=FALSE, eval=FALSE>>=
sfitL <- summary(fitL)
...
<<label=figSim2, include=FALSE, keep.source=TRUE, eval=FALSE>>=
